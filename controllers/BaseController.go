...
func (c *BaseController) JsonResult(errCode int, errMsg string, data ...interface{}) {
	...
	// 	logs.Error(err)
	// }
	// 20260109 wyf 修复 JSON marshal 失败时静默失败的问题
	if err != nil {
		logs.Error("JSON marshal failed:", err)
		// 返回 500 错误，避免静默失败
		//http.Error(c.Ctx.ResponseWriter, `{"errcode":500,"message":"Internal Server Error"}`, 500)
		c.StopRun()
		return
	}
